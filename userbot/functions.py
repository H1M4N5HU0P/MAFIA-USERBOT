...
from userbot.states import states
...
async def yt_search(mafia):
    try:
        mafia = urllib.parse.quote(mafia)
        html = urllib.request.urlopen(
            "https://www.youtube.com/results?search_query=" + mafia
        )
...
async def extract_time(mafia, time_val):
    if any(time_val.endswith(unit) for unit in ("m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]  # type: str
        if not time_num.isdigit():
            mafia.edit("Invalid time amount specified.")
            return ""
        if unit == "m":
