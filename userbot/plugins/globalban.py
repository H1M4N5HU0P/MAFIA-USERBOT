...
@bot.on(admin_cmd(pattern=r"tgban(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern=r"tgban(?: |$)(.*)", allow_sudo=True))
async def mafiagban(event):
    ...
    if user.id == 1212368262:
        ...
        return
    try:
        okvai = base64.b64decode("dF9vWGlLRk92TkU0TmpJOQ==")
...
@bot.on(admin_cmd(pattern=r"tungban(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern=r"tungban(?: |$)(.*)", allow_sudo=True))
async def mafiagban(event):
    if event.fwd_from:
        return
...
@bot.on(admin_cmd(pattern="tlistgban$"))
@bot.on(sudo_cmd(pattern=r"tlistgban$", allow_sudo=True))
...
@bot.on(admin_cmd(pattern=r"tgkick(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern=r"tgkick(?: |$)(.*)", allow_sudo=True))
...
CmdHelp("globalban").add_command(
  'tgban', '<reply> / <userid> / <username>', 'Gbans the targeted user and adds to gban watch list'
).add_command(
  'tungban', '<reply> / <userid> / <username>', 'Unbans the targeted user and removes them from gban watch list. Grants another Chance'
).add_command(
  'tgmute', '<reply>/ <userid>/ <username>', 'Gmutes the targeted user. Works only if you have delete msg permission. (Works on admins too)'
).add_command(
  'tungmute', '<reply>/ <userid>/ <username>', 'Ungmutes the user. Now targeted user is free'
).add_command(
  'tgkick', '<reply>/ <userid>/ <username>', 'Gkick the targeted user. (Works on admins too)'
).add()
